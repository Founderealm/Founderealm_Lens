...
Question: changes
Inputs: maps/files.json, dependencies.json, prior_runs
...
LENS = {'id': 'changes', 'requires': ['maps/files.json', 'dependencies.json', 'prior_runs'], 'produces': ['changes.json'], 'feeds': ['catalogue'], 'evidence': 'file_hash_and_derived_impact', 'trust_tier': 'seed_generated'}

def main():
    files = json.loads((OUT / 'maps' / 'files.json').read_text())['files']; dependencies = json.loads((OUT / 'dependencies.json').read_text()); history = _load_history(OUT / 'history' / 'runs.json')
