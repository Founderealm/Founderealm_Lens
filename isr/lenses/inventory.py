...
LENS = {'id': 'inventory', 'requires': ['repository'], 'produces': ['inventory.json'], 'feeds': ['parsing', 'changes', 'tests'], 'evidence': 'direct_filesystem_inventory', 'trust_tier': 'seed_generated'}

def main():
    dna = _dna(); _bootstrap(OUT, dna['dependencies']); sys.path.insert(0, str(OUT / 'dependencies')); importlib.invalidate_caches(); files, mode = _candidate_files(ROOT, OUT, dna); supported, unsupported = _classify_files(files, dna, _pack(OUT).detect_language_from_path)
