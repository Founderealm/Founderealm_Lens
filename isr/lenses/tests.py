...
LENS = {'id': 'tests', 'requires': ['inventory.json'], 'produces': ['tests.json'], 'feeds': ['catalogue'], 'evidence': 'directory_and_filename_convention', 'trust_tier': 'seed_generated'}

def main():
    dna = _dna(); files = [ROOT / item for item in json.loads((OUT / 'inventory.json').read_text())['files']]
